...
public class ToyMachine extends ToyFill{
    ...
    public void pollInQueue(ArrayList<ToyConstructor> toys){
        ...
        for (ToyConstructor toy : toys) {
            ...
            if (curDrop == toy.getId()){
                pq.add(toy);
                String pollMsg = "Игрушка <" + toy + "> помещена в Магазин Игрушек";
                FileWorking fw = new FileWorking();
                fw.save(pollMsg);
                System.out.println("Игрушка <" + toy + "> помещена в Магазин Игрушек");
            }    
        }
    }
    ...
    public void takeFromQueue(){
        while(!pq.isEmpty()){
            ...
            String takeMsg = "Поздравляем!Вы достали чудесную игрушку: " + curDrop;
            FileWorking fw = new FileWorking();
            fw.save(takeMsg);
